...
import { ESTADO_AUSENCIA_LABELS, type AbsenceDto, type EstadoAusencia, type HolidayDto } from '@yorga/contracts';
...
export function CalendarioAusencias({ puedeGestionar = false }: { puedeGestionar?: boolean }) {
  ...
  const load = useCallback(() => {
    ...
    // Los festivos del año se traen enteros (una llamada por año) y se filtran por mes al pintar.
    rrhhGateway.listFestivos(year).then(setFestivos).catch(() => setFestivos([]));
    rrhhGateway
  ...
  const festivoDe = (dia: number) => festivos.find((f) => f.date === iso(year, month, dia));
      ...
      <Card.Body className="p-4">
        ...
        {loading ? (
          ...
          <div className="cal-grid">
            ...
            {celdas.map((dia, i) => {
              const festivo = dia != null ? festivoDe(dia) : undefined;
              return (
              <div key={i} className={`cal-cell ${dia == null ? 'cal-empty' : ''} ${festivo ? 'cal-festivo' : ''}`} title={festivo ? `Festivo: ${festivo.name}${festivo.centerName ? ` (${festivo.centerName})` : ''}` : undefined}>
                {dia != null && (
                  <>
                    <div className="cal-num d-flex justify-content-between">{dia}{festivo && <span className="cal-festivo-dot" title={festivo.name}>★</span>}</div>
