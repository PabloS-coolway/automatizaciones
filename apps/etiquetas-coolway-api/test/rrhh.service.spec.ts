...
import { EmpleadoUpdate, EmployeeRepository, EmployeeRow } from '../src/rrhh/application/ports';
...
describe('RrhhService · alta de empleado', () => {
  it('enlaza con un usuario existente por correo, crea la ficha y lo AUDITA', async () => {
    ...
    const svc = new RrhhService(repo({ findUserIdByEmail: async () => 10 }), recorder, db);
  ...
  it('si no existe usuario con ese correo, NO crea ficha (RRHH no crea logins)', async () => {
    const svc = new RrhhService(repo({ findUserIdByEmail: async () => null }), recorderSpy().recorder, db);
  ...
  it('un usuario que ya es empleado no se duplica', async () => {
    const svc = new RrhhService(repo({ findUserIdByEmail: async () => 10, findByUserId: async () => fila({}) }), recorderSpy().recorder, db);
  ...
  it('rechaza un rol RRHH inventado', async () => {
    const svc = new RrhhService(repo({ findUserIdByEmail: async () => 10 }), recorderSpy().recorder, db);
...
describe('RrhhService · editar / baja / reactivar', () => {
  it('edita la ficha y AUDITA el antes→después', async () => {
    const { recorder, registros } = recorderSpy();
    const svc = new RrhhService(repo({ findById: async () => fila({ id: 5, fullName: 'Ana' }) }), recorder, db);
  ...
  it('editar rechaza nombre vacío y rol inventado', async () => {
    const svc = new RrhhService(repo({ findById: async () => fila({ id: 5 }) }), recorderSpy().recorder, db);
  ...
  it('editar rechaza un responsable inexistente y un empleado inexistente', async () => {
    const svc = new RrhhService(repo({ findById: async (id) => (id === 5 ? fila({ id: 5 }) : null) }), recorderSpy().recorder, db);
  ...
  it('dar de baja marca inactivo y lo audita; reactivar lo revierte', async () => {
    const { recorder, registros } = recorderSpy();
    const svc = new RrhhService(repo({ findById: async () => fila({ id: 5, active: true }) }), recorder, db);
...
describe('RrhhService · listVisible respeta la jerarquía', () => {
  ...
  const svc = new RrhhService(repo({ findAll: async () => plantilla }), recorderSpy().recorder, db);
