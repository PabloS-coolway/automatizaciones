...
export class GenerateLabelsCommand extends CommandRunner {
  constructor(@Inject(GENERATE_LABELS_USE_CASE) private readonly useCase: GenerateLabelsUseCase) {
    super();
  }
  ...
  async run(_args: string[], opts: CliOptions): Promise<void> {
    ...
    const { reconciliation: rec, missing, file } = result;
    console.log(`Pedido ${file.orderNumber}: ${file.rows.length} filas, ${rec.labelPairs} pares. Variante ${variant}${importadoPor ? `, importado por ${importadoPor}` : ''}.`);
