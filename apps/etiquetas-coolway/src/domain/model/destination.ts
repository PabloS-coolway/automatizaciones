...
export interface MarketPreset {
  /** Variante de códigos por defecto del mercado (a veces el cliente la cambia → override en CLI). */
  variant: LabelVariant;
  /** Valor de la columna "importado por" (RF-13). */
  importadoPor: string;
}
...
export function resolveMarket(code: string): MarketPreset {
  ...
  if (!preset) {
    throw new Error(`Mercado desconocido: "${code}". Válidos: ${Object.keys(MARKETS).join(', ')}`);
  }
  return preset;
}
